...
void MainWindow::initTable()
{
	...
	model.setHorizontalHeaderLabels({ "字符", "旋转角度", "X轴坐标", "Y轴坐标" });
...
void MainWindow::ImageCallBackInner(unsigned char * pData, MV_FRAME_OUT_INFO_EX* pFrameInfo)
{
	...
	stConvertParam.pDstBuffer = conv_pData;                    //ch:输出数据缓存 | en:output data buffer
	stConvertParam.nDstBufferSize = (pFrameInfo->nWidth * pFrameInfo->nHeight * 3 + 2048);            //ch:输出缓存大小 | en:output buffer size
...
void MainWindow::closeEvent(QCloseEvent *event)
{
	...
	{
		auto ret = QMessageBox::warning(NULL, "警告", "相像机未关闭，是否仍要关闭?", QMessageBox::Yes, QMessageBox::No);
...
void MainWindow::on_start_button_clicked()
{
	...
	{
		QMessageBox::information(nullptr, "提示", "未导入标定数据或导入数据不正确", QMessageBox::Ok);
		return;
	}
